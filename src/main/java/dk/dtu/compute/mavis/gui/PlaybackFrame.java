...
import dk.dtu.compute.mavis.gui.widgets.DomainPanel;
import dk.dtu.compute.mavis.gui.widgets.PlayPauseButton;
import dk.dtu.compute.mavis.gui.widgets.SeekBar;
import dk.dtu.compute.mavis.gui.widgets.SkipBackwardButton;
import dk.dtu.compute.mavis.gui.widgets.SkipForwardButton;
import dk.dtu.compute.mavis.gui.widgets.StepBackwardButton;
import dk.dtu.compute.mavis.gui.widgets.StepForwardButton;
...
class PlaybackFrame extends JFrame {
  ...
  PlaybackFrame(PlaybackManager playbackManager, Domain domain, GraphicsConfiguration gc) {
    ...
    topPanel.setBorder(
        BorderFactory.createCompoundBorder(BorderFactory.createMatteBorder(0, 0, 1, 0, borderColor),
    ...
    this.botPanel.setBorder(
        BorderFactory.createCompoundBorder(BorderFactory.createMatteBorder(1, 0, 0, 0, borderColor),
    ...
    this.speedField.addFocusListener(new FocusAdapter() {
      ...
      public void focusGained(FocusEvent e) {
        // This is silly, but required because the selection is otherwise immediately
        // reset.
        PlaybackFrame.this.speedField.setText(PlaybackFrame.this.speedField.getText());
        PlaybackFrame.this.speedField.selectAll();
      }
    });
    this.speedField.addPropertyChangeListener("value",
        e -> playbackManager.setSpeed((int) e.getNewValue()));
    ...
    StepBackwardButton stepBackwardButton =
        new StepBackwardButton(() -> playbackManager.stepBackward(1));
    ...
    StepForwardButton stepForwardButton =
        new StepForwardButton(() -> playbackManager.stepForward(1));
    ...
    globalInputMap.put(KeyStroke.getKeyStroke(KeyEvent.VK_LEFT, InputEvent.SHIFT_DOWN_MASK),
        "StepBackward10");
    globalInputMap.put(KeyStroke.getKeyStroke(KeyEvent.VK_RIGHT, InputEvent.SHIFT_DOWN_MASK),
        "StepForward10");
  ...
  void showFullscreen() {
    if (this.isDisplayable()) {
      // Store size, location, and state before disposing, so we can restore when we
      // return to
      // windowed mode.
  ...
  void showWindowed() {
    ...
    if (this.previousWindowExtendedState != -1) {
      // Frame has been shown in windowed mode before, restore size, location, and
      // state.
      ...
      // First time frame is shown in windowed mode, set default size, location, and
      // state.
      var dm = this.getGraphicsConfiguration().getDevice().getDisplayMode();
      int width = (int) (dm.getWidth() * 0.6666666666);
      int height = (int) (dm.getHeight() * 0.6666666666);
  ...
  /**
   * Called whenever frame is shown, moved, or resized. Called before going
   * borderless fullscreen mode from windowed mode. This maintains the last size
   * and location of the frame when it was in the normal state.
   */
   ...
   * content in its last rendering pass. IMPORTANT: Must only be called by the
   * EDT, after waiting on waitDomainRender().
   */
  ...
  /**
   * Signals the rendering thread for the DomainPanel to render the given state
   * interpolation.
   */
  ...
  /**
   * Wait for the DomainPanel's rendering thread to finish rendering after the
   * last call to signalDomainRender().
   */
