...
/**
 * Handles synchronization between GUIs for a given set of Domains. If
 * synchronization is not desired, instantiate one PlaybackManager per Domain
 * instead of giving several domains to one manager.
 * <p>
 * The Domain GUIs are displayed on the given GraphicsConfigurations, with the
 * initial settings given to the constructor of the PlaybackManager.
 * <p>
 * Any changes to one GUI on a manager, are reflected on the other GUIs as well.
 * (Except for the specific states rendered by the domains, which will
 * (probably) be different.)
 * <p>
 * IMPORTANT: Unless otherwise specified, all functions on PlaybackManager
 * objects MUST be called from the EDT!
 */
public class PlaybackManager {
   ...
   * ready is set to true and signaled by the EDT after it has run the constructor
   * function. running is set to true and signaled by the EDT after it has shown
   * the GUI as a result of a call to startGUI(). running is set to false and
   * signaled by the EDT after the user closes the GUI. The Main thread can wait
   * for the GUI to close in waitShutdown().
   */
  private volatile boolean ready = false;
  private volatile boolean running = true;

  /**
   * Caching number of states from each domain, for various functions. A 1-1
   * correspondence with this.domains, containing the latest results from queries
   * to domain.getNumStates().
   */
  private double[] numStates;
  private double maxNumStates;

  /**
   * The timer which triggers tick() at regular intervals for whatever FPS we
   * want. The actual FPS is determined in the constructor, based on the lowest
   * refresh rate among the display devices that we display to.
   * <p>
   * The EDT signals rendering during a tick() and then awaits all rendering
   * threads to finish, before it paints the results to the screen if necessary.
   * <p>
   * The Toolkit is cached for synchronizing buffers to underlying display every
   * tick.
   */
   ...
   * Synchronized settings among the GUIs.
   * <p>
   * The last time we started playing (in ns), and the state interpolation last
   * rendered at that time. These are used as offsets to calculate the current
   * state interpolation to render if we are playing.
   */
   ...
   * Synchronized settings among the GUIs.
   * <p>
   * The current state interpolation that we want to render next tick. If this and
   * the last rendered state interpolation are the same for a frame, then we don't
   * have to render unless DomainPanel buffers are invalidated. The DomainPanels
   * themselves take care of this, but the value must be capped to numStates for
   * the DomainPanel before requesting a render.
   */
  ...
  /**
   * Attempts to set the system look and feel. Writes a warning to
   * Server.printWarning if it fails.
   */
  ...
  /**
   * Assumes a 1-1 correspondence between the given Domains and
   * GraphicsConfigurations. I.e. domains.length == gcs.length, and gcs[i] is the
   * graphics configuration for domains[i].
   * <p>
  ...
  public PlaybackManager(Domain[] domains, GraphicsConfiguration[] gcs) {
    ...
    // Note that the actual tick rate may/will be a bit higher due to limited
  ...
  /**
   * Waits for the EDT to initialize the GUI, then shows it with the given
   * parameters. This function should only be called once. The function returns
   * after the GUI is running.
   */
  public void startGUI(boolean fullscreen, boolean hideInterface, int msPerAction,
      boolean playing) {
    synchronized (this) {
      while (!this.ready) {
        try {
  ...
  /**
   * Waits for the GUI to shut down. The function returns after the Rendering
   * threads have joined and the PlaybackFrames have been disposed. Note that
   * Swing itself may not have exited yet, but there is no apparent way to wait
   * for that.
   */
   ...
   * <p>
   * Render each DomainPanel if either of: 1. The DomainPanel has been resized. 2.
   * The associated Domain has new states and we are rendering past the end. 3. We
   * changed the current target state interpolation.
   */
  private void tick() {
    ...
    if (this.isPlaying) {
      if (this.nsPerAction == 0) {
        ...
        this.currentStateInterpolation =
            this.lastStartStateInterpolation + (double) timeSinceLastStartNs / this.nsPerAction;
        // Cap to the maximum number of states available amongst all domains.
        // Do not advance target state interpolation again until some domain exceeds the
        // current
        // maximum.
    ...
    // Update state labels.
    // Begin rendering, wait to finish, then write buffers to UI if they were
    // updated.
    for (int i = 0; i < this.frames.length; ++i) {
      ...
      String shownState =
          this.hideInterface ? String.format(Locale.ROOT, "%.3f", cappedStateInterpolation)
              : String.format(Locale.ROOT, "%.3f of %d", cappedStateInterpolation,
      ...
      String shownStateTime = this.hideInterface ? ""
          : String.format(Locale.ROOT, "%.3f s",
              this.domains[i].getStateTime((int) cappedStateInterpolation) / 1_000_000_000.0);
    ...
    if (Server.PRINT_DEBUG_FRAMETIME && elapsed / 1_000_000L > this.tickTimer.getDelay()) {
      Server.printDebug(String.format(Locale.ROOT, "Tick time (%d ms) exceeded frame time (%d ms).",
          elapsed / 1_000_000L, this.tickTimer.getDelay()));
    }
  }
}
