...
public class Server {
  public static final boolean PRINT_DEBUG = "true".equalsIgnoreCase(System.getenv("MAVIS_DEBUG"));
  public static final boolean PRINT_DEBUG_FRAMETIME =
      "true".equalsIgnoreCase(System.getenv("MAVIS_DEBUG_FRAMETIME"));
  ...
  private static void runClientOnSingleLevel(ArgumentParser args) {
    ...
    if (args.hasGUIOutput()) {
      ...
      // The domain can discard past states if we don't need to be able to seek states
      // in the GUI.
    ...
    if (args.hasLogOutput()) {
      try {
        logFileStream = Files.newOutputStream(args.getLogFilePath(), StandardOpenOption.CREATE_NEW,
    ...
    try (var levelDirectory = Files.newDirectoryStream(args.getLevelPath(), levelFileFilter)) {
      ...
      if (args.hasLogOutput()) {
        try {
          logZipStream = new ZipOutputStream(new BufferedOutputStream(Files.newOutputStream(
              args.getLogFilePath(), StandardOpenOption.CREATE_NEW, StandardOpenOption.WRITE)));
      ...
      for (Path levelPath : levelDirectory) {
        ...
        try {
          long timeoutNS = args.getTimeoutSeconds() * 1_000_000_000L;
          client =
              new Client(domain, args.getClientCommand(), logFileStream, false, timeout, timeoutNS);
        } catch (Exception e) {
          Server.printError("Could not start client process.");
          Server.printError(e.getMessage());
          continue;
      ...
      try {
        ...
        BufferedWriter logWriter = new BufferedWriter(
            new OutputStreamWriter(logFileStream, StandardCharsets.US_ASCII.newEncoder()));
  ...
  /**
   * Returns a GraphicsConfiguration array of size numScreens, attempting to map
   * the user-specified screen numbers (if provided) to the GraphicsDevices
   * corresponding to a logical left-to-right, top-to-bottom enumeration. See the
   * screen notes in ArgumentParser.
   */
  private static GraphicsConfiguration[] getGraphicsConfigurations(int numScreens, int[] screens) {
    ...
    // Attempt sorting for the logical enumeration. May not work, e.g. if bounds are
    // not relative to
    // each other.
    Arrays.sort(gds,
